 ...
 * Search Routes
 *
 * Provides API endpoints for fuzzy searching papers across multiple fields.
 * Searches: title, abstract (EN/ZH), topics, and authors with weighted relevance.
 ...
 * Global search across ALL cached papers using multi-field fuzzy search
 ...
 * Alias for /api/search/papers - explicitly named for global search
 * Global search across ALL cached papers using multi-field fuzzy search
 * Searches: title (30%), abstract (25%), abstract_zh (20%), topics (15%), authors (10%)
 * Query params:
 *   - q: Search query (required, 2-200 chars)
 ...
 *   - limit: Results per page (optional, default: 10, min: 1, max: 50)
 */
router.get('/global', searchLimiter, async (req, res) => {
  try {
    const { q, page, limit } = req.query;
    ...
    if (!q) {
      return res.status(400).json({
        error: 'Query parameter "q" is required',
        example: '/api/search/global?q=machine+learning&page=1&limit=10',
      });
    }
    ...
    // Perform search
    console.log(
      `[Global Search API] Query: "${q}", Page: ${pageNum}, Limit: ${limitNum}`
    );
    ...
    // Log search result summary
    console.log(
      `[Global Search API] Found ${results.pagination.totalResults} results in ${results.searchMetadata.executionTime}ms`
    );
  ...
  try {
    res.json({
      ...
      endpoints: {
        ...
        globalSearch: 'GET /api/search/global?q=query&page=1&limit=10',
      ...
      features: {
        ...
        searchFields: ['title', 'abstract', 'abstract_zh', 'topics', 'authors'],
        weightedRelevance: true,
        pagination: true,
        globalSearch: true,
      },
    });
